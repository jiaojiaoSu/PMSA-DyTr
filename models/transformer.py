...
class Transformer(nn.Module):
    def __init__(self, args, activation="relu"):
        ...
        if self.multiscale:
            ...
            from models.transformer_encoder import DeformableTransformerEncoder, DeformableTransformerEncoderLayer#正常结构
            # from models.transformer_encoder_multibranch import DeformableTransformerEncoder, \
            #     DeformableTransformerEncoderLayer #self-attn改进版本
            self.num_feature_levels = 4  # Hard-coded multiscale parameters modified by sjj 20221128
    ...
    def forward_multi_scale(self, srcs, masks, query_embed, pos_embeds):
        ...
        # print("srcs[0].shape:",srcs[0].shape)#([1, 256, 100, 100])
        bs, c, h_32, w_32 = srcs[1].shape
        # print("srcs[1].shape:",srcs[1].shape)#([1, 256, 50, 50])
        bs, c, h_64, w_64 = srcs[2].shape
        # print("srcs[2].shape:", srcs[2].shape)#([1, 256, 50, 50])
        bs, c, h_128, w_128 = srcs[3].shape
        # print("srcs[3].shape:", srcs[3].shape)
        ...
        src_128= srcs[3].flatten(2).permute(2, 0, 1)
        orig_pos_embed_128 = pos_embeds[3] + self.level_embed[3].unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
        pos_embed_128 = orig_pos_embed_128.flatten(2).permute(2, 0, 1)
        mask_128 = masks[3].flatten(1)
        ...
        # print("spatial_shape :", spatial_shapes)#tensor([[100, 100],[ 50,  50],[ 50,  50]], device='cuda:0')
        level_start_index = torch.cat((spatial_shapes.new_zeros((1, )), spatial_shapes.prod(1).cumsum(0)[:-1]))
        # print("level_start_index :", level_start_index)#[[    0, 10000, 12500]
        valid_ratios = torch.stack([self.get_valid_ratio(m) for m in masks], 1)
        # print(lvl_pos_embed_flatten.shape)#[2,9025,256]
        ...
        mem3 = memory[:, level_start_index[2]: level_start_index[3], :]#add
        mem4 = memory[:, level_start_index[3]: , :]#add


        memory_flatten = []
        for m in [mem1, mem2, mem3,mem4]:#add
        ...
        #add
        grid_y_128, grid_x_128 = torch.meshgrid(torch.arange(0, h_128), torch.arange(0, w_128))
        grid_128 = torch.stack((grid_x_128, grid_y_128), 2).float()
        grid_128.requires_grad = False
        grid_128 = grid_128.type_as(srcs[0])
        grid_128 = grid_128.unsqueeze(0).permute(0, 3, 1, 2).flatten(2).permute(2, 0, 1)
        grid_128 = grid_128.repeat(1, bs * 8, 1)
        #add end

        # decoder
        hs, references = self.decoder(tgt,
                                      [memory_flatten[0], memory_flatten[1], memory_flatten[2],memory_flatten[3]],
                                      memory_key_padding_mask=[mask_16, mask_32, mask_64,mask_128],
                                      pos=[pos_embed_16, pos_embed_32, pos_embed_64,pos_embed_128],
                                      query_pos=query_embed,
                                      memory_h=[h_16, h_32, h_64,h_128],
                                      memory_w=[w_16, w_32, w_64,w_128],
                                      grid=[grid_16, grid_32, grid_64,grid_128])
