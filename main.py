...
def get_args_parser():
    parser = argparse.ArgumentParser('Prior-Modulated and Semantic-Aligned Dynamic Transformer', add_help=False)
    ...
    parser.add_argument('--multiscale', default=True, action='store_true')#False->True multiscale  运用deformable detr
    # * Backbone
    ...
    parser.add_argument('--smca', default=False, action='store_true')#smca false->true
    ...
    parser.add_argument('--coco_path', type=str, default='/media/zndx/存储库/sjj/transformer-series/Deformable-DETR-main/coco_ne')# 'data/coco'
    #   u /media/zndx/存储库/sjj/yolos/YOLOS-main/path/to/coco
    ...
    parser.add_argument('--eval_every_epoch', default=1, type=int, help='eval every ? epoch')#1->2 每2个
    parser.add_argument('--save_every_epoch', default=1, type=int, help='save model weights every ? epoch')#1->10
...
def main(args):
    ...
    # add
    history_val_loss = []  # 后面调用每个epoch的情况
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_every_epoch == 0:
            ...
            if coco_evaluator is not None:
                ...
                # add-------------
            train_loss = train_stats['loss']  # add
            test_loss = test_stats['loss']  # add
            history_val_loss.append(test_loss)  # add
            if args.output_dir:
                if len(history_val_loss) <= 1 or test_loss <= min(history_val_loss):  # add best
                    checkpoint_paths.append(
                        output_dir / f'checkpoint-best.pth')
...
if __name__ == '__main__':
    # output=sys.stdout
    # outfile=open("detailed_log.txt",'w')
    # sys.stdout=outfile
